...
export default function Index() {
  ...
  const generateInterpretation = (runes: DrawnRune[], spread: RuneSpread) => {
    ...
    if (spread.id === "single") {
      ...
      text += `Шаманский бросок — древнейший способ общения с духами рун. `;
      text += `Тело (${runes[0].name}) показывает материальные обстоятельства: ${runes[0].events || "ваши физические возможности"}. `;
      text += `Душа (${runes[1].name}) открывает эмоциональное состояние: ${runes[1].relationships || "ваши чувства и связи"}. `;
      text += `Дух (${runes[2].name}) раскрывает высший смысл: ${runes[2].karmicLesson || "духовный урок ситуации"}. `;
      text += `Эти три руны работают вместе, создавая целостную картину вашей жизни в данный момент.`;
    } else if (spread.id === "thors-hammer") {
      text += `Молот Тора — мощный защитный расклад для преодоления препятствий. `;
      text += `Рукоять (${runes[0].name}) — ваша сила и ресурсы: ${runes[0].advice || "то, на что вы можете опереться"}. `;
      text += `Левая сторона (${runes[1].name}) ослабляет вас: ${runes[1].reversed || runes[1].warning || "будьте внимательны к этому аспекту"}. `;
      text += `Правая сторона (${runes[2].name}) усиливает вас: ${runes[2].upright || "используйте эту энергию"}. `;
      text += `Головка молота (${runes[3].name}) — решающий удар: ${runes[3].events || "ключевое действие"}. `;
      text += `Результат (${runes[4].name}): ${runes[4].reversed ? "потребуется дополнительная работа" : "успешное преодоление препятствия"}.`;
    } else if (spread.id === "celtic-cross") {
      text += `Кельтский крест — самый глубокий и детальный расклад. `;
      text += `Центр (${runes[0].name}) и пересечение (${runes[1].name}) показывают суть вопроса и то, что на него влияет. `;
      text += `Основа (${runes[2].name}) и прошлое (${runes[3].name}) раскрывают корни ситуации. `;
      text += `Вершина (${runes[4].name}) и будущее (${runes[5].name}) указывают возможные исходы. `;
      text += `Ваша позиция (${runes[6].name}), внешние факторы (${runes[7].name}), надежды и страхи (${runes[8].name}) формируют контекст. `;
      text += `Итог (${runes[9].name}) ${runes[9].reversed ? "требует осознанной работы для достижения" : "обещает гармоничное разрешение ситуации"}.`;
    } else if (spread.id === "personal-growth") {
      text += `Расклад личностного роста ведет вас к целостности и самопознанию. `;
      text += `Вы сейчас (${runes[0].name}): ${runes[0].personality || "ваше текущее состояние"}. `;
      text += `Ваша тень (${runes[1].name}) ${runes[1].reversed ? "особенно сильна" : "готова к принятию"}: ${runes[1].warning || "то, что вы отвергаете в себе"}. `;
      text += `Ваш свет (${runes[2].name}): ${runes[2].upright || "то, что нужно развивать и усиливать"}. `;
      text += `Урок момента (${runes[3].name}): ${runes[3].karmicLesson || "что вы должны понять сейчас"}. `;
      text += `Инструмент (${runes[4].name}): ${runes[4].advice || "как использовать эту энергию для роста"}. `;
      text += `Потенциал (${runes[5].name}): ${runes[5].energy || "кем вы можете стать, интегрировав все аспекты"}.`;
    }
